...
export default {
	...
	async execute(interaction) {
		...
		const modal = new ModalBuilder()
			.setCustomId('timeoutUserMessageModal:' + target.id + ':' + message.id)
			.setTitle('Timeout User (Referencing Message)');
	...
	async onModalSubmit(id, interaction) {
		...
		if (split[0] == 'timeoutUserMessageModal') {
